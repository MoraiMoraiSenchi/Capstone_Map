function BUS_STATION_CCTV_INFO_INIT() // BUSSTOP_INFO.json와 CCTV_INFO.json 파일 불러온 뒤 파싱하는 함수
{
    ...
    loadLIGHT(function(response){
        LIGHT_JSON = JSON.parse(response);
    });
}
...
function loadLIGHT(callback) // LIGHT_INFO.json 파일 불러오는 함수
{
    var xobj = new XMLHttpRequest();
    xobj.overrideMimeType("application/json");
    xobj.open('GET', 'LIGHT_INFO.json', true);
    xobj.onreadystatechange = function()
    {
        if(xobj.readyState == 4 && xobj.status == "200")
        {
...
function count_BUSSTOP_CCTV_LIGHT(mouse_lat, mouse_long) // 주변의 버스 정류장과 CCTV의 개수,거리를 카운트 하고 리턴하는 함수. 매개변수는 경위도.
{
    /* count_BCL 설명
        단순히 특정 반경 내 버스 정류장이나 CCTV 개수를 세는 것으로 점수를 내보니 조금 이상한 부분이 있음.
        -> 버스 정류장과 CCTV의 거리를 점수에 반영해야함
        count_BCL[0]에는 반경 500m 이내의 버스정류장 개수를 저장
        count_BCL[1]에는 반경 250m 이내의 보안등 개수를 저장
        count_BCL[2]부터 버스 정류장, CCTV, 보안등의 거리를 저장함.
    */

    var count_BCL = new Array();
    var i = 2;
    var distance = 0;
    var bus_index = 0; // 버스 정류장의 개수를 저장할 변수
    var cctv_index = 0; // CCTV 개수를 저장할 변수
